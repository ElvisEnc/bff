...
for image in $(find . -type f -iname "Dockerfile"|xargs grep -i "from "|awk '{print $2}'|sort -u| grep -Evi "${EXCLUDEIMG}"); do 
    echo "  - Scann $image"
    docker run --name ${APPALIAS} aquasec/trivy:latest image $image ${STOPCONDITION} ${ignore} --no-progress --scanners vuln --format template --timeout 10m --template @contrib/junit.tpl -o trivy_report.xml --debug
